...
export default function SubsidiariesSection({ subsidiaries, homeData }: SubsidiariesSectionProps) {
  ...
  return (
    <section className="bg-white dark:bg-[#0d0d0d] py-24 lg:py-[112px] px-6 lg:px-10 border-y border-black/10 dark:border-white/10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-15">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6">
          <h2 className="font-sans font-medium text-black dark:text-white text-3xl sm:text-4xl tracking-tight max-w-[512px]">
            {homeData?.subsidiariesHeading || "Un portefeuille construit, pas accumulé"}
          </h2>
          <p className="text-black/80 dark:text-white/90 text-lg max-w-[536px]">
            ...
            return (
              ...
              >
                ...
                <div className="p-6 flex flex-col gap-1">
                  ...
                  <h3 className="font-sans text-black dark:text-white text-xl pt-1">{item.name}</h3>

                  <div className="flex items-center gap-3 pt-1 text-xs">
                    {item.participationLabel && (
                      <span className="text-black/80 dark:text-white/80">
                        Participation :{" "}
                        <span className="text-black dark:text-white font-medium">{item.participationLabel}</span>
                      </span>
                    )}
                    {item.entryYear && (
                      <>
                        ...
                        <span className="text-black/80 dark:text-white/80">
                          Entrée <span className="text-black dark:text-white">{item.entryYear}</span>
                        </span>
                      </>
                    )}
                  </div>

                  <p className="text-black/80 dark:text-white/80 text-base leading-relaxed pt-4">{item.shortDescription}</p>

                  <div className="flex items-center justify-between pt-4 mt-4 border-t border-black/[0.06] dark:border-white/[0.06]">
                    <span className="text-black/80 dark:text-white/80 text-xs">Voir la fiche</span>
                    <CtaArrow
                      size={14}
        ...
        <div className="flex flex-col gap-6 pt-4">
          <h3 className="font-sans font-medium text-black dark:text-white text-3xl sm:text-4xl tracking-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {synergies.map((syn) => (
              <div
                key={syn.id}
                className="bg-black/[0.01] dark:bg-white/[0.01] border border-black/[0.08] dark:border-white/[0.06] rounded-lg p-5"
              >
                <p className="text-sm">
                ...
                <p className="text-black/80 dark:text-white/80 text-sm leading-relaxed pt-2">{syn.description}</p>
              </div>
            ))}
          </div>
        </div>
