      ...
      <div className="max-w-[1280px] mx-auto flex flex-col lg:flex-row">
        ...
        <div className="flex-1">
          {items.map((item, idx) => {
            ...
            return (
              <div key={item.id ?? idx} className="border-b border-black/[0.05] dark:border-white/[0.08]">
                <button
                ...
                >
                  <div className="flex-1 flex flex-col gap-4">
                    <h3 className="font-inter font-medium text-black dark:text-white text-lg sm:text-xl">{item.question}</h3>
                    {isOpen && item.answer && (
                      typeof item.answer === "string" ? (
                        <p className="font-inter font-medium text-[#333] dark:text-white/70 text-sm leading-relaxed">{item.answer}</p>
                      ) : (
                        <RichTextRenderer data={item.answer} className="font-inter text-[#333] dark:text-white/70 text-sm" />
                      )
                    )}
                  </div>
