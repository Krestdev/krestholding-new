...
export default function InvestmentProcessSection({ homeData }: InvestmentProcessSectionProps) {
  ...
  return (
    <section id="notre-modele" className="bg-white dark:bg-[#0d0d0d] py-24 lg:py-[112px] px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-16">
        <div className="grid grid-cols-1 lg:grid-cols-[389px_1fr] gap-10 lg:gap-16">
          <div className="flex flex-col gap-6">
            ...
            <h2 className="font-sans font-medium text-[#010101] dark:text-white text-3xl sm:text-4xl tracking-tight">
              {homeData?.modelHeading || "Notre modèle en 3 temps"}
            </h2>
            <p className="text-[#878887] dark:text-white/60 text-sm leading-relaxed">
              {homeData?.modelBody ||
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {modelSteps.map((step, idx) => (
              <div
                key={idx}
                className="relative border-t-2 border-x border-b border-black/[0.08] dark:border-white/[0.12] p-6 flex flex-col"
              >
                <span className="text-[#218da8]/25 font-abel text-6xl leading-none pb-2">
                  {String(idx + 1).padStart(2, "0")}
                </span>
                <h3 className="font-sans text-[#010101] dark:text-white text-lg pt-4">{step.title}</h3>
                <p className="text-[#878887] dark:text-white/60 text-sm leading-relaxed pt-3">{step.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8">
          <h3 className="font-sans font-medium text-[#111] dark:text-white text-xl sm:text-2xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {processSteps.map((step, idx) => (
              <div key={idx} className="flex flex-col items-center gap-3 text-center">
                ...
                <p className="font-sans text-[#010101] dark:text-white text-base">{step.title}</p>
        ...
        <div className="bg-black/[0.02] dark:bg-white/[0.03] border border-black/[0.06] dark:border-white/[0.08] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 p-6">
          <div className="flex flex-col gap-1">
            <p className="font-sans text-[#010101] dark:text-white text-base">
              {homeData?.processNoteTitle || "Votre dossier est traité en toute confidentialité."}
            </p>
            <p className="text-[#878887] dark:text-white/60 text-sm">
              {homeData?.processNoteBody || "Réponse systématique sous 15 jours ouvrés."}
            </p>
          </div>
          <Link
            href={homeData?.processCtaUrl || "#soumettre-dossier"}
            className="group inline-flex items-center gap-2.5 pl-6 pr-3.5 py-3 bg-black text-white dark:bg-white dark:text-black text-sm font-medium shrink-0 transition-colors hover:opacity-90"
          >
