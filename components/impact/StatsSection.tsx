...
export default function StatsSection({ pageData }: StatsSectionProps) {
  ...
  return (
    <section id="impact-en-chiffres" className="bg-white dark:bg-[#0d0d0d] pt-16 lg:pt-[120px] pb-16 px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-10">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-20">
          <div className="flex flex-col gap-6 lg:w-[467px] shrink-0">
            <h2 className="font-sans text-black dark:text-white text-3xl sm:text-4xl leading-tight">
              {pageData?.statsHeading || "Investir avec KREST, c'est investir dans l'Afrique de demain"}
            </h2>
            <p className="text-black/90 dark:text-white/90 text-base leading-relaxed">
              {pageData?.statsIntro ||
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-10 flex-1 lg:pt-[74px]">
            {stats.map((stat, idx) => (
              <div key={stat.id ?? idx} className="flex flex-col gap-4">
                <span className="font-sans text-black dark:text-white text-4xl sm:text-5xl tracking-tight">{stat.value}</span>
                <span className="font-abel text-black/64 dark:text-white/64 text-sm uppercase tracking-wide">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
