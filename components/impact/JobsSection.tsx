...
export default function JobsSection({ pageData }: JobsSectionProps) {
  ...
  return (
    <section id="emplois-competences" className="bg-white dark:bg-[#0d0d0d] py-16 lg:py-[120px] px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-14">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-6">
          <div className="flex flex-col gap-12">
            <h3 className="font-sans text-black dark:text-white text-2xl">
            ...
            <div className="flex flex-col gap-9">
              {chart.map((row, idx) => (
                <div key={row.id ?? idx} className="flex items-center gap-6 w-full">
                  <span className="text-black dark:text-white text-sm w-[160px] shrink-0 truncate">{row.label}</span>
                  <div className="flex-1 h-[11px] bg-black/32 dark:bg-white/32">
                    <div
                      className="h-full bg-[#f29308]"
                      style={{ width: `${Math.max(0, Math.min(100, (row.value / max) * 100))}%` }}
                    />
                  </div>
                  <span className="text-black dark:text-white text-sm w-4 text-right shrink-0">{row.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {cards.map((card, idx) => (
              <div key={card.id ?? idx} className="bg-black/[0.08] dark:bg-white/[0.08] border border-black/30 dark:border-white/30 p-6 flex flex-col gap-3">
                <h4 className="text-black dark:text-white text-sm font-medium">{card.title}</h4>
                <p className="text-black/70 dark:text-white/70 text-xs leading-relaxed">{card.description}</p>
              </div>
            ))}
          </div>
        </div>
