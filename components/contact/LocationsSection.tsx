    ...
    <section className="bg-[#0d0d0d] py-24 lg:py-[120px] px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {groups.map((group) => (
            <div key={group.city} className="bg-white/[0.08] border border-white/30 p-6 flex flex-col gap-12">
                ...
                {group.entries.map((entry) =>
                  entry.subsidiary ? (
                    <Link
                      ...
                      className="text-white text-xl underline hover:text-white/80 transition-colors"
                    >
                      {entry.name.toUpperCase()}
                    </Link>
                  ) : (
                    <span key={entry.name} className="text-white text-xl underline decoration-white/32">
                      {entry.name.toUpperCase()}
                    </span>
                  ),
                )}
