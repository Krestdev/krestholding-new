    ...
    <section className="bg-[#0d0d0d] py-24 lg:py-[120px] px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-20">
        <div className="flex items-center gap-4">
          ...
          <span className="font-abel text-white text-xl uppercase tracking-tight">Coordonnées & siège</span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8">
          ...
          <p className="text-white/90 text-lg sm:text-xl leading-snug max-w-[514px]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <div className="border border-white/24 flex flex-col">
            {rows.map((row, idx) => (
              <div
                key={row.label}
                className={`flex items-center justify-between gap-4 p-6 ${idx % 2 === 0 ? "bg-white/[0.12]" : ""}`}
              >
                <span className="text-white/64 text-base">{row.label}</span>
                <span className="text-white text-base text-right">{row.value}</span>
              </div>
            ))}
            <div className="flex items-center justify-between gap-4 p-6 bg-white/[0.12]">
              <span className="text-white/64 text-base">Nos réseaux sociaux</span>
              <div className="flex items-center gap-4">
                {contactData?.linkedinUrl && (
                  <a href={contactData.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-white">
                    <LinkedinLogo size={24} />
                  </a>
                )}
                {contactData?.twitterUrl && (
                  <a href={contactData.twitterUrl} target="_blank" rel="noopener noreferrer" className="text-white">
                    <XLogo size={24} />
                  </a>
                )}
                {!contactData?.linkedinUrl && !contactData?.twitterUrl && (
                  <>
                    <LinkedinLogo size={24} className="text-white/32" />
                    <XLogo size={24} className="text-white/32" />
                  </>
                )}
              </div>
            </div>
            <div className="flex items-center justify-between gap-4 p-6">
              <span className="text-white/64 text-base">Horaires</span>
              <span className="text-white text-base text-right">{hours}</span>
            </div>
          </div>
          ...
          <div className="relative min-h-[320px] flex flex-col justify-end gap-4 p-6 overflow-hidden">
            ...
            <span className="absolute top-6 right-6 backdrop-blur-md bg-black/32 border border-white/12 px-4 py-2.5 text-sm font-medium text-white">
              Zone d&apos;intervention
            </span>
            <div className="relative flex items-center justify-between gap-4">
              <div className="flex flex-wrap gap-2.5">
                {zones.map((zone) => (
                  <span
                    key={zone}
                    className="backdrop-blur-md bg-black/32 border border-white/12 px-4 py-2.5 text-sm font-medium text-white uppercase tracking-wide"
                  >
                    {zone}
                  </span>
                ))}
              ...
              {contactData?.directionsUrl && (
                <a
                  ...
                  className="inline-flex items-center gap-2 backdrop-blur-md bg-black/32 border border-white/12 px-4 py-2.5 text-sm font-medium text-white uppercase tracking-wide shrink-0"
                >
