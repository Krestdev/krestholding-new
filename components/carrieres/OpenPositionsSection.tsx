...
const DEFAULT_JOBS: JobOpening[] = [
  {
    ...
    contractType: "CDI",
    location: "Douala",
    description: "Pilotage de chantiers, coordination des équipes techniques et suivi budgétaire des projets BTP.",
  ...
  },
  {
    ...
    contractType: "CDI",
    location: "Yaoundé",
    description: "Conception et développement des applications métier internes du groupe.",
    publishedAt: new Date().toISOString(),
    applicationDeadline: new Date().toISOString(),
  },
  {
    ...
    contractType: "CDI",
    location: "Yaoundé",
    description: "Gestion opérationnelle et animation d'équipe pour un établissement de restauration premium.",
    publishedAt: new Date().toISOString(),
    applicationDeadline: new Date().toISOString(),
  },
];
...
export default function OpenPositionsSection({ pageData, jobs }: OpenPositionsSectionProps) {
  ...
  const totalPages = useMemo(() => Math.max(1, Math.ceil(items.length / PER_PAGE)), [items.length]);
  const currentPage = Math.min(page, totalPages);
  const pageItems = items.slice((currentPage - 1) * PER_PAGE, currentPage * PER_PAGE);

  return (
    <section id="nos-offres" className="bg-white py-16 lg:py-[120px] px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {pageItems.map((job) => {
            ...
            return (
              <div key={job.id} className="border border-black/10 p-6 flex flex-col gap-6">
                {published && <span className="text-black/40 text-[10px] uppercase tracking-wide">Publiée le {published}</span>}
                <div className="flex flex-col gap-3">
                  <h4 className="font-sans text-[#111] text-lg leading-snug">{job.title}</h4>
                  ...
                  <p className="text-black/64 text-sm leading-relaxed">{job.description}</p>
                </div>
                {deadline && <span className="text-black/40 text-xs">Date limite : {deadline}</span>}
              </div>
            );
          })}
        </div>
