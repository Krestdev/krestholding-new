...
export default function RecruitmentProcessSection({ pageData }: RecruitmentProcessSectionProps) {
  ...
  return (
    <section id="notre-processus" className="bg-[#0d0d0d] py-16 lg:py-[120px] px-6 lg:px-10">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-14">
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8">
          ...
          <p className="text-white/64 text-base max-w-[500px]">
            {pageData?.processIntro || "Un processus clair, rapide, et une réponse systématique à chaque candidature."}
          </p>
        </div>

        <h3 className="font-sans text-white text-2xl sm:text-3xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 lg:gap-4 pt-4">
          {steps.map((step, idx) => (
            <div key={step.id ?? idx} className="relative border border-white/16 p-6 pt-8 flex flex-col gap-3">
              ...
              <p className="text-white/64 text-sm leading-relaxed">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
