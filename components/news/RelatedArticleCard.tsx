...
export default function RelatedArticleCard({ item }: RelatedArticleCardProps) {
      ...
      <div className="flex items-end gap-4 p-6">
        <div className="flex-1 flex flex-col gap-3">
          <div className="flex items-center gap-4 font-mono text-[10px] uppercase">
            ...
            <span className="text-[#878887] dark:text-white">{item.category || "Communiqué"}</span>
          </div>
          <p className="font-inter font-medium text-black dark:text-white text-base leading-snug line-clamp-3">
            {item.title}
          </p>
        </div>
